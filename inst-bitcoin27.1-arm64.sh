#!/bin/bash
#Instalazione di bitcoin-27.1-aarch64-linux-gnu (x raspi)
#eseguire in $HOME
#Colori
co1="\e[0;31m\033[1m"
co2="\e[0;33m\033[1m"
...
function_dowload_btc(){
   ...
   wget https://bitcoincore.org/bin/bitcoin-core-27.1/bitcoin-27.1-aarch64-linux-gnu.tar.gz > /dev/null
   echo -e "${co5}[+] ${endco}${co8}Scarica checksum crittografico SHA256SUMS ${endco}"
   wget https://bitcoincore.org/bin/bitcoin-core-27.1/SHA256SUMS > /dev/null
   echo -e "${co5}[+] ${endco}${co8}Scarica firme checksum ${endco}"
   wget https://bitcoincore.org/bin/bitcoin-core-27.1/SHA256SUMS.asc > /dev/null 
...
function_unzip_clean(){
   ...
   echo -e "\n${co5}[+] ${endco}${co8}Decomprime Bitcoin Core 27.1${endco}"
   tar xzf bitcoin-27.1-aarch64-linux-gnu.tar.gz -v
   sleep 1
   #pulire files
   echo -e "${co5}[+] ${endco}${co8}Elimina files temporanei ${endco}"
   rm -r -f SHA256SUMS  SHA256SUMS.asc  bitcoin-27.1-aarch64-linux-gnu.tar.gz  guix.sigs
   sleep 1 
}
